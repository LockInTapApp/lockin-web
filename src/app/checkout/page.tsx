...
const PAYMENTS = [
  { id: "bank_transfer", label: "Bank Transfer", sub: "BCA · Mandiri · BNI · BRI" },
...
export default function CheckoutPage() {
  ...
  const applyPromo = () => {
    ...
    if (code === "LOCKIN10" || code === "FOCUS20") {
      ...
      toast.error("Invalid promo code", { description: "Try LOCKIN10 or FOCUS20" });
    }
  };
  ...
  const handlePay = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Couldn't place order", { description: "Please try again." });
    } finally {
      setLoading(false);
    }
  };
  ...
  if (success) {
    return (
      <div className="min-h-screen bg-white text-neutral-900" data-testid="checkout-success-page">
  ...
  return (
    <div data-testid="checkout-page" className="min-h-screen bg-white text-neutral-900">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-10 md:py-14 grid lg:grid-cols-12 gap-10 lg:gap-16">
        <form onSubmit={handlePay} className="lg:col-span-7 space-y-10" data-testid="checkout-form">
          <div>
            <h1 className="font-display text-3xl md:text-4xl font-light tracking-tighter">Checkout</h1>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <RadioGroupItem id={`pay-${p.id}`} value={p.id} className="text-emerald-600" />
                      <div>
                        <div className="text-sm font-medium text-neutral-900">{p.label}</div>
                        <div className="text-xs text-neutral-500 mt-0.5">{p.sub}</div>
                      </div>
                    </div>
          ...
          <div className="lg:sticky lg:top-24 rounded-3xl border border-neutral-200 bg-neutral-50 p-6 md:p-8">
            ...
            <div className="flex gap-4">
              <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden border border-neutral-200 bg-white flex-shrink-0">
                ...
                <img src={PRODUCT_IMG} alt="Lockin Classic" className="w-full h-full object-cover" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-display text-lg font-medium text-neutral-900">
                  Lockin <span className="text-neutral-500">Classic</span>
                </div>
                <div className="text-xs text-neutral-500 mt-1">Anodized aluminium · NFC</div>
                ...
                <div className="mt-3 inline-flex items-center gap-3 bg-white border border-neutral-200 rounded-full px-1.5 py-1">
                  ...
                  <span data-testid="qty-display" className="text-sm font-medium w-5 text-center">
                    {qty}
                  </span>
                  <button
                    type="button"
...
function SectionHeader({ index, title }: { index: string; title: string }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <h2 className="font-display text-xl font-medium text-neutral-900">{title}</h2>
    </div>
  );
}
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between" data-testid={testId}>
      ...
      <span className={accent ? "text-emerald-700" : "text-neutral-900"}>{value}</span>
    </div>
  );
}
